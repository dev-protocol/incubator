...
contract LiquidityIncentiveStorage is UsingStorage {
	...
	// withdrawn
	function setWithdrawn(address _provider, uint256 _value) internal {
		bytes32 key = getWithdrawnKey(_provider);
		eternalStorage().setUint(key, _value);
	}

	function getWithdrawn(address _provider) public view returns (uint256) {
		bytes32 key = getWithdrawnKey(_provider);
		return eternalStorage().getUint(key);
	}

	function getWithdrawnKey(address _provider) private pure returns (bytes32) {
		return keccak256(abi.encodePacked("_withdrawn", _provider));
	}
}
